...
{#if oracle.isOpen}
  ...
  >
    ...
    >
      ...
      {#if oracle.progress}
        <div class="text-xs font-mono text-purple-400/70 text-center py-2 animate-pulse">
          [{oracle.progress}]
        </div>
      {/if}
    </div>
    ...
    <div class="p-3 border-t border-purple-900/30 bg-black/50">
      <form onsubmit={(e) => { e.preventDefault(); handleSubmit(); }} class="flex gap-2">
        <input
          ...
          disabled={oracle.isLoading && !oracle.progress} 
        />
        <button
          type="submit"
          ...
          disabled={!input.trim() || (oracle.isLoading && !oracle.progress)}
        >
          ➤
        </button>
      </form>
