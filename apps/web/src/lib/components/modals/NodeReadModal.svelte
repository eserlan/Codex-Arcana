<script lang="ts">
  ...
  const entity = $derived(ui.readModeNodeId ? vault.entities[ui.readModeNodeId] : null);
...
{#if ui.readModeNodeId}
  ...
  >
    ...
    >
        <!-- Header -->
        <div class="p-6 border-b border-green-900/30 flex justify-between items-start bg-[#0a0a0a]">
            <div>
                {#if entity}
                    <div class="flex items-center gap-3 mb-2">
                        <span class="{getIconClass(categories.getCategory(entity.type)?.icon)} text-green-500 w-5 h-5"></span>
                        <span class="text-xs font-bold tracking-widest text-green-600 uppercase">{entity.type}</span>
                    </div>
                    <h2 class="text-2xl md:text-3xl font-bold text-gray-100 font-serif tracking-wide">{entity.title}</h2>
                {:else}
                ...
                {#if entity}
                    ...
                    >
                        ...
                        {:else if copyStatus === 'error'}
                            <span class="icon-[lucide--alert-triangle] w-6 h-6 text-red-500"></span>
                        {:else}
        ...
        <div class="flex-1 overflow-y-auto p-8 custom-scrollbar">
            {#if entity}
                ...
                {#if entity.tags && entity.tags.length > 0}
                    <div class="flex flex-wrap gap-2 mb-6">
                        {#each entity.tags as tag}
                            <span class="text-[10px] font-mono px-2 py-1 bg-green-900/20 text-green-400 border border-green-900/30 rounded">#{tag}</span>
                        {/each}
                    </div>
                {/if}
                ...
                <!-- Main Content -->
                <div class="prose prose-invert prose-lg max-w-none text-gray-200 mb-12">
                    {#if renderedContent}
                ...
                <div class="border-t border-green-900/30 pt-6">
                    <h3 class="text-green-500 font-serif italic text-xl mb-4">Connections</h3>
                    {#if connections.length > 0}
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            {#each connections as conn}
                                ...
                                >
                                    <span class="w-4 h-4 shrink-0 {conn.isOutbound ? 'text-green-500 icon-[lucide--arrow-up-right]' : 'text-blue-500 icon-[lucide--arrow-down-left]'}"></span>
                                    <div class="flex-1 min-w-0">
                                        <div class="text-sm font-bold text-gray-200 group-hover:text-green-400 transition truncate">{conn.displayTitle}</div>
                                        <div class="text-xs text-gray-500 truncate">{conn.label || conn.type}</div>
                                    </div>
                                </button>
                            {/each}
                        </div>
            ...
            {:else}
                <p class="text-gray-500 font-mono">The requested node could not be loaded.</p>
            {/if}
        </div>
    </div>
  </div>
